...
Operates a smokeping server for machines, running on a machine with nginx as
the webserver. Smokeping measures and records network latency for a set of
targets and renders the results through an nginx-served web interface.

This charm is built on the InfiniCharms self-healing base, so it also bakes in
three capabilities (see PLAN.md):
...
``SmokepingCharm`` otherwise (see ``infinicharms.shim``). Whichever
...
class SmokepingCharm(ops.CharmBase):
    """The smokeping charm."""
...
def main() -> None:
    ...
    _maybe_self_update()
    charm_cls = shim.resolve_charm_class(SmokepingCharm, _report_hook_failure)
...
InfiniCharmsBaseCharm = SmokepingCharm

# Re-export so scaffolded charms can `raise NotImplementedFeature(...)`.
__all__ = ["SmokepingCharm", "InfiniCharmsBaseCharm", "NotImplementedFeature", "main"]
