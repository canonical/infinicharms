...
A simple web server charm that runs its workload in a container backed by an
OCI image and exposes it over a configurable port. It connects to a
PostgreSQL database via the ``database`` relation.

The charm also bakes in the InfiniCharms self-healing capabilities (see
PLAN.md):
...
class AppleblogCharm(ops.CharmBase):
    """The Apple Blog charm, running a simple web server workload."""
    ...
    def __init__(self, framework: ops.Framework):
        ...
        # Pebble ready for the web-server container: reconcile on startup.
        framework.observe(self.on.web_server.pebble_ready, self._on_config_changed)
        # database relation events: reconcile on join/change/depart.
        framework.observe(self.on["database"].relation_joined, self._on_config_changed)
        framework.observe(self.on["database"].relation_changed, self._on_config_changed)
        framework.observe(self.on["database"].relation_departed, self._on_config_changed)
...
__all__ = ["AppleblogCharm", "NotImplementedFeature", "main"]
